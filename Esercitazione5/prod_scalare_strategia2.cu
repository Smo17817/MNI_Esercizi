...
__global__ void ProdGPU_Reduction(float *v, const float *a, const float *b, int N) {
    ...
    /* Riduzione in memoria condivisa - Strategia II per la somma */
    ...
    for(int k = 0; k < passi; k++) {
        int dist = 1 << k; // 2^k
        int temp = 1 << (k + 1); // 2^(k+1)

        // Calcolo del resto=(threadIdx.x, 2^(k+1))
        resto = menum % temp;

        if(resto == 0) {
            recvBy = menum + dist;
            // Controllo per non uscire dai limiti del blocco
            if(recvBy < nthrd) {
                s[menum] += s[recvBy];
            }
        }
